...
ps auwx | grep -i jscover | grep -v grep | awk '{print $2;}' | perl -ne 'chomp; if($_ ne "") { `kill -9 $_`; }'
...
java -jar src/lib/jscover/JSCover-all.jar -ws --document-root=/var/www/regula --report-dir=target &
